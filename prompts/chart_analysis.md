...
3. 후보별 기술적 점검 (구글 파이낸스 + WebSearch "{종목명} 주가 차트 분석"):
   **편입 기준은 "상승여지가 확인되는 저평가 종목"이다. 다음을 순서대로 점검한다:**
   - 상승여지 (필수): 52주 범위 내 현재 위치가 하단~중단부에 있고, 위로 열린 공간이
     있어야 한다. 52주 최고가 대비 -10% 이내(신고가 부근)는 상승여지 부족 → reject
   - 하락 추세 함정 확인 (필수): 싸다고 사는 게 아니다. 최저가 부근이라면 "떨어지는
     칼"인지 "바닥 다지기"인지 구분한다 — 최근 캔들이 저점을 높이고 있거나, 거래량이
     실리며 반등 시작한 증거가 있어야 enter 가능. 계속 저점을 낮추는 중이면 watch
   - 당일 캔들: 시가/고가/저가/종가 관계 (장대양봉? 위꼬리? 갭상승?)
   - 거래량: 평균 거래량 대비 당일 거래량 — 바닥권에서 거래량 급증 + 양봉이면 가점
   - price_history가 5일 이상 쌓인 종목은 단기 추세(5일 이동 방향)도 계산
   - 판단: enter(편입) / watch(관망, 내일 재검토) / reject(탈락) + 근거.
     enter에는 반드시 "왜 지금이 상승 초입인가"를 technical_notes에 쓴다
